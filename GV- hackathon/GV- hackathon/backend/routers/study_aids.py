...
async def flashcards(doc_id: str, page_id: int):
    try:
        logger.info(f"🎴 Flashcards request: doc_id={doc_id}, page_id={page_id}")
        ...
        items = generate_flashcards(page_text)
...
async def quiz(doc_id: str, page_id: int):
    try:
        logger.info(f"📝 Quiz request: doc_id={doc_id}, page_id={page_id}")
        doc = doc_store.get(doc_id)
        if not doc:
            logger.warning(f"⚠️ Document not found: {doc_id}")
            raise HTTPException(status_code=404, detail="Document not found")
...
async def cheatsheet(doc_id: str, page_id: int):
    try:
        logger.info(f"📋 Cheatsheet request: doc_id={doc_id}, page_id={page_id}")
        doc = doc_store.get(doc_id)
        if not doc:
            logger.warning(f"⚠️ Document not found: {doc_id}")
            raise HTTPException(status_code=404, detail="Document not found")
        ...
        content = generate_cheatsheet(page_text)
