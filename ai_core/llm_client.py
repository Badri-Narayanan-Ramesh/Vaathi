...
def explain_page_voice(page_context: str) -> Dict[str, object]:
    ...
    app_mode = os.getenv("APP_MODE", "local")
    model = os.getenv("GEMINI_MODEL" if app_mode == "cloud" else "OLLAMA_MODEL", "default")
    logger.debug(f"explain_page_voice: app_mode={app_mode}, model={model}, cache_key={key[1][:16]}")
    if key in _CACHE:
        logger.debug(f"explain_page_voice: CACHE HIT")
        raw = _CACHE[key]
        return {"text": raw, "raw": raw, "meta": {"cached": True}}

    logger.debug(f"explain_page_voice: CACHE MISS, generating")
    resp = llm_client.generate_with_metadata(prompt, sys_prompt=sys_prompt)
    text = resp.get("text", "")
    _CACHE[key] = text
    logger.debug(f"explain_page_voice: cached result with key={key[1][:16]}")
...
def answer_question_voice(question: str, ctx_texts: List[str]) -> Dict[str, object]:
    ...
    app_mode = os.getenv("APP_MODE", "local")
    model = os.getenv("GEMINI_MODEL" if app_mode == "cloud" else "OLLAMA_MODEL", "default")
    logger.debug(f"answer_question_voice: app_mode={app_mode}, model={model}, cache_key={key[1][:16]}")
    if key in _CACHE:
        logger.debug(f"answer_question_voice: CACHE HIT")
        raw = _CACHE[key]
        return {"text": raw, "raw": raw, "meta": {"cached": True}}

    logger.debug(f"answer_question_voice: CACHE MISS, generating")
    style = " ".join(VOICE_ANSWER_SHORT)
    sys_prompt = render_voice_prompt(style)
    resp = llm_client.generate_with_metadata(prompt, sys_prompt=sys_prompt)
    text = resp.get("text", "")
    _CACHE[key] = text
    logger.debug(f"answer_question_voice: cached result with key={key[1][:16]}")
    return {"text": text, "raw": resp.get("raw"), "meta": resp.get("meta")}